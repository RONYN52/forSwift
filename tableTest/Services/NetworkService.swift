...
class NetworkService {    
    func loadWineByColors(sendText: String, token: String, completion: @escaping (WineData?) -> Void) {
        guard let url = URL(string: URLs.getURL + "/wineCategory/" + sendText+"/0/15")
        else {
            return
        }
    ...
    func loadSVGByWine(sendText: String, completion: @escaping (UIImage?) -> Void) {
    ...
    func loadSummaryOfWine(wineId: String, token: String, completion: @escaping (FullWineData?) -> Void) {
